...
public class BallPanel extends JPanel implements KeyListener, ActionListener, MouseListener {
    ...
    ArrayList<GameObject> lgo; //Lista di tutti i GameObjects.GameObject che sono presenti
    ...
    /**
     * Riceve le info su lingua e livello che sono decise nel Engine.MainFrame
     ...
     * @param parent Engine.MainFrame a cui fa riferimento
     */
    ...
    /**
     * Metodo che determina se bisogna muovere la scena a destra in base alla posizione e alla velocità della GameObjects.Ball
     *
     */
    public void leftBound() {
        Ball b = (Ball) lgo.get(0);
        if (b.getX() <= 300 && b.getSpeedX() < 0) {
            scrollingL = true;
            moveSceneRight();
    ...
    /**
     * Metodo che determina se bisogna muovere la scena a sinistra in base alla posizione e alla velocità della GameObjects.Ball
     *
     */
    public void rightBound() {
        Ball b = (Ball) lgo.get(0);
        if (b.getX() + b.getW() >= this.getWidth() - 300 && b.getSpeedX() > 0) {
            scrollingR = true;
            moveSceneLeft();
    ...
    /**
     * Metodo che determina se bisogna muovere la scena verso l'alto in base alla posizione e alla velocità della GameObjects.Ball
     *
     */
    public void upperBound() {
        Ball b = (Ball) lgo.get(0);
        if (b.getY() < 300 && b.getSpeedY() < 0) {
            scrollingU = true;
            moveSceneUp();
    ...
    /**
     * Metodo che determina se bisogna muovere la scena verso il basso in base alla posizione e alla velocità della GameObjects.Ball
     *
     */
    public void lowerBound() {
        Ball b = (Ball) lgo.get(0);
        groundUpdate();
        if ((b.getY() > this.getHeight() - 100 && b.getSpeedY() > 0) && ground < this.getHeight()) {
            scrollingD = true;
            moveSceneDown();
    ...
    /**
     * Contiene un ciclo che controlla lo stato degli oggetti GameObjects.Fuoco, in cui quelli non più esistenti vengono rimossi dalla lgo
     */
    ...
    /**
     * Contiene un ciclo che controlla lo stato degli oggetti GameObjects.Nemico, in cui quelli non più esistenti vengono rimossi dalla lgo
     */
    ...
    /**
     * Contiene un ciclo che controlla lo stato degli oggetti GameObjects.PowerUp, in cui quelli non più esistenti vengono rimossi dalla lgo
     */
    void powerUpUpdate() {
        Iterator<GameObject> i = lgo.iterator();
        while (i.hasNext()) {
    ...
    public void paintPause(Graphics g) {
        if (pause) {
            timer.stop();
            PTimer.start();
            if (!gameOver && !lvSuperato && !guida.isShowing()) {
                if (animation) {
                    pauseImage.paintIcon(this, g, 0, 0);
                } else {
                    pauseResume.paintIcon(this, g, 0, 0);
                }
            } else if (gameOver && !guida.isShowing()) {
                paintGameOver(g);
            } else if (!guida.isShowing()) {
    ...
    /**
     * Controlla se la GameObjects.Ball ha 0 vite, per determinare l'eventuale gameOver.
     */
    void checkDeath() {
    ...
    public double getSceneSpeedX() {
        return sceneSpeedX;
    }

    public double getSceneSpeedY() {
        return sceneSpeedY;
    }

    public void setLvSuperato(boolean lvSuperato) {
        this.lvSuperato = lvSuperato;
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getX() >= getWidth() - 100 && e.getY() <= 100) {
            ...
            if (!guida.isShowing()) {
                guida.setShowing(true);
                pause = true;
            } else {
                pause = false;
                guida.setShowing(false);
                timer.start();
            }
        }
    }
